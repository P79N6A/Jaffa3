...
const defaultSafebrowsingServer = "api.whitehat.ro"
const defaultSafebrowsingURL = "https://%s/dns/lookup?prefixes=%s"
const defaultParentalServer = "api.whitehat.ro"
const defaultParentalURL = "https://%s/dns/parental/lookup?prefixes=%s&sensitivity=%d"
...
func (d *Dnsfilter) CheckHost(host string) (Result, error) {
	...
	if result.Reason.Matched() {
		//log.Printf("host \"%s\" mathed. Rule \"%s\", reason: %v", host, result.Rule, result.Reason)
		return result, nil
	}
...
func hostnameToHashParam(host string, addslash bool) (string, map[string]bool) {
	...
	for {
		...
		//log.Printf("tzais: %s - %s - %s - %s - %s", hashparam.String(), hashes, curhost, pos, tld)
		if pos < 0 {
			break
		}
